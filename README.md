...
```bash
yarn add @nhost/react-auth @nhost/react-apollo @apollo/client graphql graphql-tag
```

`@apollo/client` is required for `@nhost/react-apollo` to work. You will use `@apollo/client` for `useQuery`, `useMutation`, `useSubscription`.
